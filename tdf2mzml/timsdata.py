#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Python wrapper for timsdata library."""
...
if sys.platform[:5] == "win32":
    libname = Path(__file__).with_name("timsdata.dll").as_posix()
elif sys.platform[:5] == "linux":
    libname = Path(__file__).with_name("libtimsdata.so").as_posix()
else:
...
dll.tims_extract_centroided_spectrum_for_frame.argtypes = [
    c_uint64,
    c_int64,
    c_uint32,
    c_uint32,
...
class TimsData:
    ...
    def extractProfileForFrame(self, frame_id, scan_begin, scan_end):
        ...
        rc = self.dll.tims_extract_profile_for_frame(
            self.handle, frame_id, scan_begin, scan_end, callback_for_dll, None
        )  # python does not need the additional context, we have nonlocal
