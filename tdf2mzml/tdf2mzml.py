#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Convert Bruker TDF files to mzML."""
...
from tdf2mzml import __version__, timsdata
...
def scan_progress(mzml_data_struct, interval=1000):
    ...
    Logs progress for writing mzml spectra.  Defaults to output a log message
    every 1000 scans.
    ...
    interval " int
        logging interval, defaults to 1000
    Returns
    -------
    None
    """
    if mzml_data_struct["scan_index"] % int(interval) == 0:
        end_time = time.time()
        logging.info(
            "Processed {} of {} scans in {:.3f} s".format(
...
def sha1_checksum(path_name):
    """
    Determine and return SHA1 checksum for file
...
def centroid_precursor_frame(mzml_data_struct):
    ...
    num_scans = (
        mzml_data_struct["td"]
        .conn.execute(
            "SELECT NumScans FROM Frames WHERE Id={0}".format(precursor_frame_id)
        )
        .fetchone()[0]
    )
    # print(
    #     mzml_data_struct["td"]
    #     .conn.execute(
    #         "SELECT NumScans FROM Frames WHERE Id={}".format(precursor_frame_id)
    #     )
    #     .fetchone()
    # )
    data_list = mzml_data_struct["td"].extractCentroidedSpectrumForFrame(
        precursor_frame_id, 0, num_scans
    )
...
def profile_precursor_frame(mzml_data_struct):
    ...
    i_array = mzml_data_struct["td"].extractProfileForFrame(
        precursor_frame_id, 0, num_scans
    )
    ...
    m_array = mzml_data_struct["td"].indexToMz(
        precursor_frame_id, list(range(0, len(i_array)))
    )
...
def get_spectrum_dict(mzml_data_struct):
    ...
    spectrum_dict["acq_software_version"] = (
        mzml_data_struct["td"]
        .conn.execute(
    ...
    spectrum_dict["instrument_serial_number"] = (
        mzml_data_struct["td"]
        .conn.execute(
    ...
    spectrum_dict["mz_acq_range_lower"] = (
        mzml_data_struct["td"]
        .conn.execute('Select Value from GlobalMetadata where key="MzAcqRangeLower"')
        .fetchone()[0]
    )

    spectrum_dict["mz_acq_range_upper"] = (
        mzml_data_struct["td"]
        .conn.execute('Select Value from GlobalMetadata where key="MzAcqRangeUpper"')
        .fetchone()[0]
    )

    spectrum_dict["frame_count"] = (
        mzml_data_struct["td"].conn.execute("SELECT COUNT(*) FROM Frames").fetchone()[0]
    )
    ...
    MMsMsType0_frames = len(
        mzml_data_struct["td"]
        .conn.execute("SELECT Id , Time From Frames where MsMsType=0")
        .fetchall()
    )
    ...
    MMsMsType8_frames = len(
        mzml_data_struct["td"]
        .conn.execute("SELECT Id , Time From Frames where MsMsType=8")
        .fetchall()
    )
    ...
    if MMsMsType8_frames > 0:
        MMsMsType8_spectra = (
            mzml_data_struct["td"]
            .conn.execute("SELECT COUNT(*) FROM Precursors")
            .fetchone()[0]
        )
    ...
    MMsMsType9_frames = len(
        mzml_data_struct["td"]
        .conn.execute("SELECT Id , Time From Frames where MsMsType=9")
        .fetchall()
    )
    ...
    spectrum_dict["total_spectra"] = (
        MMsMsType0_spectra + MMsMsType8_spectra + MMsMsType9_frames
    )
...
#         #Get number of MS2 Spectra for each precursors in a give Frame
...
def write_sourcefile_list(mzml_data_struct):
    ...
    mzml_data_struct["writer"].file_description(
        ["MS1 spectrum", "MSn spectrum"], sf_list
    )
    return
...
def write_reference_param_group_list(mzml_data_struct):
    """
    Writes the referencable param group list
    ...
    software_list.append(
        {
            "id": "TIMS_SDK",
            "version": "2.8.7",
            "params": [{"Bruker software": ""}, {"software name": "TIMS SDK"}],
        }
    )
...
def write_instrument_configuration_list(mzml_data_struct):
    ...
    detector = mzml_data_struct["writer"].Detector(
        3, ["microchannel plate detector", "photomultiplier"]
    )
    serial_number = mzml_data_struct["data_dict"]["instrument_serial_number"]

    instparams = [{"instrument serial number": serial_number}]

    if mzml_data_struct["data_dict"]["PASEFMSMS_type9"] == 1:
        windows = (
            mzml_data_struct["td"]
            .conn.execute("SELECT * From DiaFrameMsMsWindows")
            .fetchall()
        )
    ...
    instrument_configurations.append(
        mzml_data_struct["writer"].InstrumentConfiguration(
            id="IC1", component_list=[source, analyzer, detector], params=instparams
        )
    )
...
def write_data_processing_list(mzml_data_struct):
    ...
    methods.append(
        mzml_data_struct["writer"].ProcessingMethod(
            order=0, software_reference="tdf2mzml", params=["Conversion to mzML"]
        )
    )

    data_processing.append(
        mzml_data_struct["writer"].DataProcessing(
            methods, id="tdf2mzml_Bruker_conversion"
        )
    )
...
def write_precursor_frame(mzml_data_struct):
    ...
    mzml_data_struct["current_precursor"]["spectrum_id"] = "index={}".format(
        mzml_data_struct["scan_index"]
    )
...
def get_precursor_list(mzml_data_struct):
    ...
    """
    # get MS2 spectrum for each precursor
    precursor_frame_id = mzml_data_struct["current_precursor"]["id"]
    ...
    precursor_list = (
        ...
        .conn.execute(
            "Select {} From Precursors where Parent={}".format(
                ",".join(precursor_columns), precursor_frame_id
            )
        )
        .fetchall()
    )
...
def write_pasef_msms_spectrum(mzml_data_struct):
    ...
    """
    precursor = {
        precursor_columns[i]: mzml_data_struct["current_precursor"]["data"][i]
        for i in range(len(precursor_columns))
    }
    ...
    parent_frame_list = (
        mzml_data_struct["td"]
        .conn.execute(
            "SELECT Frame From PasefFrameMsMsInfo where Precursor={}".format(
                precursor["Id"]
            )
        )
        .fetchall()
    ...
    pasef_frame_info_list = (
        mzml_data_struct["td"]
        .conn.execute(
            "SELECT {} From PasefFrameMsMsInfo where Precursor={}".format(
                ",".join(pasef_frame_columns), precursor["Id"]
            )
        )
        .fetchone()
    )
    ...
    pasef_frame = {
        pasef_frame_columns[i]: pasef_frame_info_list[i]
        for i in range(len(pasef_frame_columns))
    }

    precursor_info = dict()

    ion_mobility = mzml_data_struct["td"].scanNumToOneOverK0(
        precursor["Parent"], [precursor["ScanNumber"]]
    )[0]
    precursor_info["params"] = [
        {"inverse reduced ion mobility": ion_mobility, "unit_accession": "MS:1002814"}
    ]

    precursor_info["spectrum_reference"] = mzml_data_struct["current_precursor"][
        "spectrum_id"
    ]
    ...
    if precursor["Charge"] is not None:
        ...
        precursor_info["isolation_window_args"]["lower"] = (
            isolation_width - isolation_offset
        )
        precursor_info["isolation_window_args"]["upper"] = (
            isolation_width + isolation_offset
        )
    ...
    mzml_data_struct["writer"].write_spectrum(
        ...
        params=[{"ms level": 2}, {"total ion current": ms2_i_array.sum()}],
    )
...
def write_pasef_dia_spectrum(mzml_data_struct):
    ...
    result = mzml_data_struct["td"].extractCentroidedSpectrumForFrame(
        frame_id, start_scan, end_scan
    )
    ...
    # # TODO find the correct metadata and apply it here properly
    precursor_info["activation"] = ["CID", {"collision energy": collision_energy}]
    precursor_info["isolation_window_args"] = dict()
...
def write_mzml(args):
    ...
    Parameters
    ----------
    args : args namespace object from argparse
    ...
    logging.info(
        "{} Total Frames.".format(mzml_data_struct["data_dict"]["frame_count"])
    )
    logging.info(
        "{} Total Spectra.".format(mzml_data_struct["data_dict"]["total_spectra"])
    )
    logging.info(
        "{} MS1 Spectra.".format(mzml_data_struct["data_dict"]["ms1_spectra_count"])
    )
    logging.info(
        "{} MS2 DDA Scans.".format(mzml_data_struct["data_dict"]["ms2_spectra_count"])
    )
    logging.info(
        "{} MS2 DIA Scans.".format(mzml_data_struct["data_dict"]["dia_spectra_count"])
    )
    ...
    logging.info("Reading, merging and formatting frames for mzML")
    ...
    with mzml_data_struct["writer"].run(
        ...
        with mzml_data_struct["writer"].spectrum_list(count=total_spectra_count):
            ...
            if mzml_data_struct["data_dict"]["PASEFMSMS_type8"] == 1:
                mzml_data_struct["precursor_frames"] = (
                    mzml_data_struct["td"]
                    .conn.execute("SELECT * From Frames where MsMsType=0")
                    .fetchall()
                )
                # Check upper frame range
                if (
                    mzml_data_struct["end_frame"] == -1
                    or mzml_data_struct["end_frame"]
                ...
                for precursor_frame in mzml_data_struct["precursor_frames"]:
                    ...
                    mzml_data_struct["current_precursor"]["start_time"] = (
                        precursor_frame[1] / 60
                    )

                    if (
                        mzml_data_struct["current_precursor"]["id"]
                        < mzml_data_struct["start_frame"]
                        or mzml_data_struct["current_precursor"]["id"]
                        > mzml_data_struct["end_frame"]
                    ):
                        continue
                ...
                mzml_data_struct["dia_frames"] = (
                    mzml_data_struct["td"]
                    .conn.execute("SELECT * From Frames where MsMsType=9")
                    .fetchall()
                )
                mzml_frames = (
                    mzml_data_struct["td"]
                    .conn.execute("SELECT * From Frames")
                    .fetchall()
                )
                # Check upper frame range
                if (
                ...
                for frame in mzml_frames:
                    ...
                    if frame[4] == 0:
                        ...
                        window_group = (
                            mzml_data_struct["td"]
                            .conn.execute(
                                "SELECT WindowGroup From DiaFrameMsMsInfo where Frame={}".format(
                                    frame[0]
                                )
                            )
                            .fetchone()[0]
                        ...
                        mzml_data_struct["current_dia_frame"]["start_time"] = (
                            frame[1] / 60
                        )
                        mzml_data_struct["current_dia_frame"][
                            "window_group"
...
def main():
    ...
    parser.add_argument(
        "-i", "--input", action="store", type=str, metavar="input_file", required=True
    )
